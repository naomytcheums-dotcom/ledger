...
ledger is that principle as a small, dependency-free library instead
of a one-off fix.

Six pieces:
- `record`     -- DecisionRecord: one decision, fully attributed, with
                  its own tamper-evidence hash.
...
- `query`      -- audit queries over a loaded ledger: by prompt
                  version, by model version, by data snapshot, by time
                  range.
